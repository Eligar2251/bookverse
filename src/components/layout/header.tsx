...
import {
  Search, Menu, BookOpen, LayoutDashboard, User, LogOut,
  Library, PenTool, TrendingUp, BookMarked, Feather,
} from 'lucide-react'
    ...
    <header className="sticky top-0 z-50 w-full border-b bg-card/80 backdrop-blur-md supports-[backdrop-filter]:bg-card/60">
      <div className="container flex h-14 items-center gap-4 px-4 mx-auto max-w-7xl">
                ...
                <div className="border-t pt-4">
                  <nav className="flex flex-col gap-1">
                    <Link href="/dashboard" onClick={() => setMobileOpen(false)}
                      ...
                      <LayoutDashboard className="h-4 w-4" />Панель автора
                    </Link>
        ...
        <Link href="/" className="flex items-center gap-2 font-bold text-lg shrink-0">
          ...
          <span className="hidden sm:inline tracking-tight">BookVerse</span>
        </Link>
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          {loading ? (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/dashboard" className="cursor-pointer"><LayoutDashboard className="mr-2 h-4 w-4" />Панель автора</Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link href="/library" className="cursor-pointer"><Library className="mr-2 h-4 w-4" />Моя библиотека</Link>
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link href={`/profile/${profile.username}`} className="cursor-pointer"><User className="mr-2 h-4 w-4" />Мой профиль</Link>
                  </DropdownMenuItem>
                  {(profile.role === 'author' || profile.role === 'admin') && (
                    <DropdownMenuItem asChild>
                      <Link href="/dashboard/books" className="cursor-pointer"><PenTool className="mr-2 h-4 w-4" />Мои книги</Link>
                    </DropdownMenuItem>
                  )}
                  <DropdownMenuSeparator />
            ...
            <div className="flex items-center gap-2">
              ...
              <Link href="/register">
                <Button size="sm" className="font-medium">Регистрация</Button>
              </Link>
            </div>
          )}
        </div>
